...
export async function postGpsMetricsByEntityId(
  ...
  try {
    ...
    if (error instanceof Exception) {
      ...
      response.send(error).status(HttpStatus.UNKNOWN_ERROR);
    }
  }
}
...
export async function postGeofenceByEntityId(
  ...
  try {
    ...
    const entityId = request.body.entityId;
    const dimensions = request.body.dimensions.map((point: [number, number]) =>
      point.join(" "),
    );
    const usableDimensions = dimensions.join(", ");

    const postGeofenceQuery = `INSERT
        INTO geofence
        (name, dimensions, entity_id)
        VALUES
        (?, ST_GeomFromText('POLYGON((${usableDimensions}))'), ?)`;
